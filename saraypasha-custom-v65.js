(function () {
  ...
  function placeBankPortal(form,panel){
    ...
    panel.classList.remove("sp-bank-inline-panel");
    if(panel.parentElement!==document.body) document.body.appendChild(panel);
    if(!bankPositionEventsBound){
      bankPositionEventsBound=true;
      window.addEventListener("resize",function(){placeBankPortal(activeBankForm,activeBankPanel);});
    }
    var rect=form.getBoundingClientRect();
    var panelHeight=panel.getBoundingClientRect().height||267;
    form.style.setProperty("--sp-bank-panel-space",Math.ceil(panelHeight+16)+"px");
    panel.style.left=Math.round(rect.left)+"px";
    panel.style.top=Math.round(rect.top-panelHeight-16)+"px";
    panel.style.width=Math.round(rect.width)+"px";
    requestAnimationFrame(function(){
      if(!document.contains(form)||!document.contains(panel)) return;
      var freshRect=form.getBoundingClientRect();
      var freshHeight=panel.getBoundingClientRect().height||panelHeight;
      form.style.setProperty("--sp-bank-panel-space",Math.ceil(freshHeight+16)+"px");
      panel.style.left=Math.round(freshRect.left)+"px";
      panel.style.top=Math.round(freshRect.top-freshHeight-16)+"px";
      panel.style.width=Math.round(freshRect.width)+"px";
    });
  }
  ...
  function enhanceBankTransfer(){
    ...
    Array.prototype.slice.call(document.querySelectorAll("form")).forEach(function(form){
      ...
      loadBankAccount().then(function(account){
        ...
        var accountKey=account.active?("active|"+account.account_name+"|"+account.iban):"inactive";
        if(panel.dataset.spAccountKey!==accountKey){
          ...
          if(!account.active){
            ...
            panel.innerHTML='<div class="sp-bank-limits"><span>İŞLEM LİMİTLERİ</span><div><strong>Minimum Yatırım</strong><b>3.000 TRY</b></div><div><strong>Maksimum Yatırım</strong><b>1.000.000 TRY</b></div><div class="sp-bank-fast-logo"><img src="https://cdn.jsdelivr.net/gh/ceibkese/SarayPashaCasino@main/fast-logo-v1.png?v=1" alt="FAST"></div></div>'+
  ...
  function enhanceAnnouncementBar(){
    if(!window.matchMedia("(max-width: 768px)").matches){
      var desktopCustom=document.querySelector(".sp-stable-announcement");
      if(desktopCustom) desktopCustom.remove();
      return;
    }
    var nativeAnnouncement=document.querySelector('[data-mj="announcement"]');
    var nativeLine=nativeAnnouncement&&nativeAnnouncement.querySelector("p");
    var nativeText=nativeLine&&(nativeLine.textContent||"").trim();

    /* Yalnizca duyuru metnini sakla. Eski sayfanin tasiyici/header kutusunu
       kopyalamak hamburger alanini bozdugu icin artik hicbir zaman tasimiyoruz. */
    if(nativeText){
      var cleanLine=nativeLine.cloneNode(true);
      var walker=document.createTreeWalker(cleanLine,NodeFilter.SHOW_TEXT);
      var textNode=walker.nextNode();
      if(textNode) textNode.nodeValue=textNode.nodeValue.replace(/^\s*(?:📢|📣|🔊)\s*/,"");
      try{sessionStorage.setItem("spAnnouncementLineHtml",cleanLine.innerHTML);}catch(e){}
    }

    var sports=/^\/(tr|en)\/sport(?:\/|$)/i.test(window.location.pathname);
    var custom=document.querySelector(".sp-stable-announcement");
    if(sports){
      if(custom) custom.remove();
      /* Yerel duyuru Spor rotasinda da header icinde kalir. Metin ve animasyon
         sinifi kaybedilmez; boylece Ana Sayfa donusunde ayni dugum kullanilir. */
      if(nativeAnnouncement&&nativeLine&&nativeText){
        nativeLine.classList.add("sp-announcement-line");
      }
      return;
    }

    /* Platformun kendi duyurusu mevcutsa ona hic dokunma. Yedek duyuru
       yalnizca SPA gecisinden sonra yerel duyuru gercekten kaybolursa kullanilir. */
    if(nativeAnnouncement&&nativeText){
      if(custom) custom.remove();
      nativeLine.classList.add("sp-announcement-line");
      return;
    }
    ...
    var lineHtml="";
    try{
      lineHtml=sessionStorage.getItem("spAnnouncementLineHtml")||"";
      if(!lineHtml){
        var legacyHtml=sessionStorage.getItem("spAnnouncementTemplate")||"";
        if(legacyHtml){
          var legacyBox=document.createElement("div");
          legacyBox.innerHTML=legacyHtml;
          var legacyLine=legacyBox.querySelector("p");
          if(legacyLine) lineHtml=legacyLine.innerHTML;
        }
      }
    }catch(e){}
    if(!lineHtml) return;

    /* Spor modulu SPA gecisinde duyuru dugumunu tutup sadece metnini
       bosaltabiliyor. Ana sayfaya donuste ayni yerel satiri canlandir. */
    if(nativeAnnouncement&&nativeLine){
      nativeLine.innerHTML=lineHtml;
      nativeLine.classList.add("sp-announcement-line");
      if(custom) custom.remove();
    ...
    /* Her rota degisiminde o anki gercek header bulunur. Duyuru eski bir
       parent/wrapper yerine header navigasyonunun hemen onune eklenir. */
    var headerNav=document.querySelector('[data-mj="header-nav"]');
    var header=(headerNav&&headerNav.closest("header"))||document.querySelector("header");
    if(!header) return;

    if(!custom||custom.parentElement!==header){
      if(custom) custom.remove();
      custom=document.createElement("div");
      custom.className="sp-stable-announcement";
      custom.innerHTML='<p class="sp-announcement-line"></p>';
      header.insertBefore(custom,header.firstChild);
    }
    var customLine=custom.querySelector("p");
    if(customLine&&customLine.innerHTML!==lineHtml) customLine.innerHTML=lineHtml;
  }
  ...
  function refresh(){ syncPageMode(); enhanceBankTransfer(); enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); enhanceQuickActions(); enhanceProviderShortcuts(); enhanceBannerDots(); enhanceAnnouncementBar(); watchWithdrawalPanel(); watchDepositPanel(); continuePendingAction(); }
  ...
  function refreshAfterRouteChange(){
    ...
    /* Mobil SPA gecisinde duyuru dugumu korunurken animasyon Spor ekraninda
       ilerlemeye devam eder. Yeni rota yerlestikten sonra yerel satiri yeniden
       baslatmak, Ana Sayfa donusunde metnin ekran disinda kalmasini engeller. */
    setTimeout(restartAnnouncementMarquee,90);
    setTimeout(restartAnnouncementMarquee,360);
  }
