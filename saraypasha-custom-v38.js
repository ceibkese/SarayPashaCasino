(function () {
  ...
  function enhanceAnnouncementBar(){
    ...
    /* Platformun kendi duyurusu gorunmez; bos tasiyicisi de yer kaplamaz. */
    if(nativeAnnouncement){
      var nativeHost=nativeAnnouncement.parentElement;
      nativeAnnouncement.classList.add("sp-native-announcement");
      if(nativeHost&&!nativeHost.querySelector('[data-mj="header-nav"]')) nativeHost.classList.add("sp-native-announcement-host");
    }
    ...
    var lineHtml="";
    try{lineHtml=sessionStorage.getItem("spAnnouncementLineHtml")||"";}catch(e){}
    if(!lineHtml) return;
    ...
    if(!headerNav||!headerShell) return;

    if(!custom||custom.parentElement!==headerShell){
      if(custom) custom.remove();
      custom=document.createElement("div");
      custom.className="sp-stable-announcement";
      custom.innerHTML='<p class="sp-announcement-line"></p>';
      headerShell.insertBefore(custom,headerNav);
    }
