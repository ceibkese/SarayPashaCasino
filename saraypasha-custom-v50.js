(function () {
  ...
  function formatBankTransferLimits(){
    Array.prototype.slice.call(document.querySelectorAll("p,span")).forEach(function(element){
      if(element.children.length) return;
      var text=normalizeTurkish(element.textContent||"").replace(/\s+/g," ").trim();
      if(/^MIN\.?\s*3[.,]?000\s*TRY$/.test(text)){
        element.textContent="Minimum Yatırım: 3.000 TRY";
        element.classList.add("sp-bank-method-limit","sp-bank-method-limit-min");
      }else if(/^MAKS\.?\s*1[.,]?000[.,]?000\s*TRY$/.test(text)){
        element.textContent="Maksimum Yatırım: 1.000.000 TRY";
        element.classList.add("sp-bank-method-limit","sp-bank-method-limit-max");
      }
    });
  }
  ...
  function enhanceBankTransfer(){
    Array.prototype.slice.call(document.querySelectorAll("form")).forEach(function(form){
      var formText=normalizeTurkish(form.textContent||"");
      if(formText.indexOf("MANUAL HAVALE")===-1&&formText.indexOf("BANK TRANSFER")===-1&&formText.indexOf("BANKA HAVALESI")===-1) return;
      ...
      formatBankTransferLimits();
      Array.prototype.slice.call(form.querySelectorAll("p,span,div")).forEach(function(element){
        if(element.children.length) return;
        if(normalizeTurkish(element.textContent||"").replace(/\s+/g," ").trim()==="MANUAL HAVALE") element.classList.add("sp-bank-native-title");
      });
      ...
      loadBankAccount().then(function(account){
        ...
        var panel=form.querySelector(".sp-bank-account-panel");
        if(!panel){
        ...
        if(!account.active){
          ...
          panel.innerHTML='<div class="sp-bank-instruction">Gönderiminizi yaptıktan sonra tutarı girerek talep iletiniz.</div>'+
        ...
        if(submit){submit.textContent="TALEP İLET";submit.disabled=!account.active;submit.setAttribute("aria-disabled",account.active?"false":"true");}
      }).catch(function(){
        var panel=form.querySelector(".sp-bank-account-panel");
        if(!panel){panel=document.createElement("section");panel.className="sp-bank-account-panel";form.insertBefore(panel,form.firstChild);}
        panel.innerHTML='<div class="sp-bank-unavailable">Banka hesabı bilgisi şu anda alınamıyor. Lütfen kısa süre sonra tekrar deneyiniz.</div>';
  ...
  if(window.visualViewport&&!window.__spDepositViewportLockReady){
    ...
    var onDepositViewportChange=function(){setTimeout(clampDepositScroll,0);};
  ...
  function refresh(){ syncPageMode(); enhanceBankTransfer(); enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); enhanceQuickActions(); enhanceProviderShortcuts(); enhanceBannerDots(); enhanceAnnouncementBar(); watchWithdrawalPanel(); continuePendingAction(); }
