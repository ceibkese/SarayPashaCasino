(function () {
  ...
  function readLoggedInUsername(){
    ...
    for(var i=0;i<labels.length;i++){
      var label=labels[i];
      if(label.children.length===0 && (label.textContent||"").trim().toLocaleLowerCase("tr-TR")==="kullanıcı adı:"){
        var sibling=label.nextElementSibling;
        var value=sibling ? (sibling.textContent||"").trim() : "";
        if(/^[A-Za-z0-9._-]{3,50}$/.test(value)) return value;
      }
    }
    var match=(document.body.innerText||"").match(/Kullanıcı Adı:\s*([A-Za-z0-9._-]{3,50})/i);
    return match ? match[1] : "";
  }
  ...
  function enhanceBonusRequestPage(){
    ...
    if(content.dataset.spBonusEmbedReady==="1") return;
  ...
  function refresh(){ enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); }
