(function () {
  ...
  function enhanceWithdrawalFrames(){
    ...
    if(found){
      document.documentElement.classList.remove("sp-withdrawal-preparing");
      if(document.documentElement.classList.contains("sp-withdrawal-method-switching")){
        var selectedIds=getSelectedPaymentIds();
        var strictId=selectedIds.filter(function(id){return ["113","410"].indexOf(id)!==-1;})[0]||"";
        if(strictId&&isStrictWithdrawalFormReady(strictId)){
  ...
  var timer=0; refreshObserver=new MutationObserver(function(){
    if(timer) return;
    timer=setTimeout(function(){timer=0;refresh();},24);
  });
