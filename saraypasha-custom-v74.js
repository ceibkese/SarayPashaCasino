(function () {
  ...
  function enhanceBankTransfer(){
    ...
    if(!isBankTransferDepositRoute()||!isBankTransferSelected()){
  ...
  function clearBankWithdrawalState(){
    Array.prototype.slice.call(document.querySelectorAll("form.sp-bank-withdrawal-form")).forEach(function(form){
      ...
      var head=form.querySelector(".sp-bank-withdrawal-head");
      if(head) head.remove();
      Array.prototype.slice.call(form.querySelectorAll(".sp-bank-withdrawal-title")).forEach(function(item){item.classList.remove("sp-bank-withdrawal-title");});
  ...
  function enhanceBankWithdrawal(){
    ...
    Array.prototype.slice.call(document.querySelectorAll("form")).forEach(function(form){
      ...
      iban.placeholder="TR ile başlayan IBAN numaranızı giriniz";
      bank.placeholder="Banka adını giriniz";
      Array.prototype.slice.call(form.querySelectorAll("p,span,div")).forEach(function(element){
        if(element.children.length) return;
        var title=normalizeTurkish(element.textContent||"").replace(/\s+/g," ").trim();
        if(title==="MANUAL HAVALE"||title==="BANK TRANSFER"||title==="BANKA HAVALESI") element.classList.add("sp-bank-withdrawal-title");
      });
      if(!form.querySelector(".sp-bank-withdrawal-head")){
        var limits=readBankTransferLimits();
        var head=document.createElement("section");
        head.className="sp-bank-withdrawal-head";
        head.innerHTML='<div class="sp-bank-withdrawal-heading">BANKA HAVALESİ İLE ÇEKİM</div>'+
          '<div class="sp-bank-withdrawal-copy">Çekimin yapılacağı IBAN ve banka bilgilerini eksiksiz giriniz.</div>'+
          '<div class="sp-bank-withdrawal-limits"><div><small>Minimum Çekim</small><strong></strong></div><div><small>Maksimum Çekim</small><strong></strong></div></div>';
        var values=head.querySelectorAll("strong");
        values[0].textContent=limits.minimum||"2.000 TRY";
        values[1].textContent=limits.maximum||"500.000 TRY";
        form.insertBefore(head,form.firstChild);
      }
      var submit=form.querySelector('button[type="submit"]');
      if(submit){
        submit.classList.add("sp-bank-withdrawal-submit");
        submit.setAttribute("data-sp-label","ÇEKİM TALEBİ İLET");
      }
    });
  ...
  function refresh(){ syncPageMode(); syncSportsIframeHeight(); enhanceBankTransfer(); enhanceBankWithdrawal(); enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); enhanceQuickActions(); enhanceProviderShortcuts(); enhanceBannerDots(); enhanceAnnouncementBar(); watchWithdrawalPanel(); watchDepositPanel(); continuePendingAction(); }
