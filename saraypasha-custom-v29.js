(function () {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .sp-bonus-embed-pane{display:flex!important;flex-direction:column!important;min-width:0!important;min-height:0!important;overflow:hidden!important}
      .sp-bonus-embed-content{display:flex!important;flex:1 1 auto!important;min-width:0!important;min-height:0!important;padding:0!important;overflow:hidden!important;background:#101d08!important}
      ...
      @media(max-width:768px){.sp-bonus-embed-frame{min-height:calc(100dvh - 58px)!important}}
      ...
      @media(max-width:768px){
        ...
        .sp-provider-shortcuts{width:calc(100% - 40px)!important;margin:7px auto 18px!important;gap:7px!important}
        .sp-provider-shortcut{display:block!important;position:relative!important;min-height:0!important;height:auto!important;padding:0!important;overflow:hidden!important;border:1px solid rgba(221,205,125,.13)!important;border-radius:8px!important;background:none!important;clip-path:polygon(10px 0,100% 0,100% 100%,10px 100%,0 calc(100% - 10px),0 10px)!important;box-shadow:0 7px 18px rgba(0,0,0,.24),inset 0 1px rgba(255,255,255,.08)!important}
        ...
        .sp-provider-shortcut img{display:block!important;width:100%!important;height:auto!important;margin:0!important;object-fit:contain!important}
        .sp-provider-play-pill{position:absolute!important;z-index:3!important;right:8px!important;top:50%!important;display:flex!important;align-items:center!important;justify-content:center!important;gap:5px!important;min-width:104px!important;height:34px!important;padding:0 11px!important;transform:translateY(-50%)!important;border:1px solid rgba(207,224,126,.2)!important;border-radius:18px!important;background:linear-gradient(180deg,rgba(50,72,15,.96),rgba(23,43,5,.96))!important;box-shadow:0 4px 11px rgba(0,0,0,.3),inset 0 1px rgba(255,255,255,.08)!important;color:#fff!important;font-size:12px!important;font-weight:800!important;line-height:1!important;white-space:nowrap!important}
        .sp-provider-play-pill b{color:#49ff20!important;font-size:21px!important;line-height:1!important}
        .sp-banner-dots{display:flex!important;align-items:center!important;justify-content:center!important;gap:9px!important;width:100%!important;height:18px!important;margin:-43px 0 0!important;padding:0!important}
        .sp-banner-dot{display:block!important;width:8px!important;height:8px!important;margin:0!important;padding:0!important;border:0!important;border-radius:50%!important;background:rgba(194,207,123,.25)!important;box-shadow:none!important;transition:transform .18s ease,background .18s ease!important}
        .sp-banner-dot.sp-banner-dot-active{background:#e3c764!important;transform:scale(1.18)!important;box-shadow:0 0 8px rgba(227,199,100,.38)!important}
      }
    `;
  ...
  function readLoggedInUsername(){
    ...
    if(!isValidUsernameCandidate(savedUsername)){
  ...
  var bonusUsernameCaptureInProgress=false;
  ...
  function openBonusPageAfterUsernameCapture(){
    if(readLoggedInUsername()){
      navigateToBonusRequest();
      return;
    }
    if(bonusUsernameCaptureInProgress) return;
    bonusUsernameCaptureInProgress=true;
    var profileButton=document.querySelector('button[aria-label="user"],button[aria-label="User"],[role="button"][aria-label="user"]');
    if(profileButton) profileButton.click();
    var attempts=0;
    var captureTimer=setInterval(function(){
      attempts+=1;
      var username=readLoggedInUsername();
      if(username||attempts>=30){
        clearInterval(captureTimer);
        bonusUsernameCaptureInProgress=false;
        if(!username) showToast("Kullanıcı adınız otomatik okunamadı. Bonus sayfasında kullanıcı adınızı kontrol ediniz.");
        navigateToBonusRequest();
      }
    },100);
  }
  ...
  function enhanceProviderShortcuts(){
    ...
    root.innerHTML=
      '<a class="sp-provider-shortcut sp-provider-pragmatic" href="/'+language+'/casino?provider=pragmatic-play"><img src="https://cdn.jsdelivr.net/gh/ceibkese/SarayPashaCasino@main/pragmatic-play-hemen-oyna.png" alt="Pragmatic Play"><span class="sp-provider-play-pill"><b>➜</b> Hemen Oyna!</span></a>'+ 
      '<a class="sp-provider-shortcut sp-provider-egt" href="/'+language+'/casino?provider=egt-digital"><img src="https://cdn.jsdelivr.net/gh/ceibkese/SarayPashaCasino@main/egt-digital-bell-link-hemen-oyna.png" alt="EGT Digital Bell Link"><span class="sp-provider-play-pill"><b>➜</b> Hemen Oyna!</span></a>'+ 
      '<a class="sp-provider-shortcut sp-provider-amusnet" href="/'+language+'/casino?provider=amusnet"><img src="https://cdn.jsdelivr.net/gh/ceibkese/SarayPashaCasino@main/amusnet-egt-hemen-oyna.png" alt="Amusnet EGT"><span class="sp-provider-play-pill"><b>➜</b> Hemen Oyna!</span></a>';
  ...
  function refresh(){ syncPageMode(); enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); enhanceQuickActions(); enhanceProviderShortcuts(); enhanceBannerDots(); continuePendingAction(); }

  function refreshAfterRouteChange(){
    syncPageMode();
    if(isHomePage()){setTimeout(refresh,60);}else{removeHomeBannerExtras();}
  }

  ["pushState","replaceState"].forEach(function(method){
    var original=history[method];
    if(typeof original!=="function") return;
    history[method]=function(){var result=original.apply(this,arguments);refreshAfterRouteChange();return result;};
  });
  ...
  setTimeout(refresh,700); setTimeout(refresh,1800); setInterval(syncPageMode,500);
