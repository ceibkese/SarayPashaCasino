(function () {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .sp-quick-actions{position:fixed!important;right:18px!important;bottom:86px!important;z-index:2147483000!important;display:flex!important;flex-direction:column!important;align-items:flex-end!important;gap:10px!important;font-family:inherit!important;pointer-events:none!important}
      ...
      .sp-quick-action-main{display:grid!important;place-items:center!important;width:58px!important;height:58px!important;padding:0!important;border:1px solid rgba(244,213,104,.9)!important;border-radius:50%!important;background:radial-gradient(circle at 34% 27%,#f6df83 0,#c8992f 31%,#74500d 72%,#3f2b08 100%)!important;box-shadow:0 9px 28px rgba(0,0,0,.4),0 0 0 4px rgba(126,177,24,.16),inset 0 2px 5px rgba(255,255,255,.38)!important;color:#1b2c04!important;font-family:Georgia,serif!important;font-size:31px!important;font-weight:900!important;line-height:1!important;text-shadow:0 1px rgba(255,255,255,.5)!important;cursor:pointer!important;pointer-events:auto!important;transition:transform .2s ease,filter .2s ease!important}
      ...
      @media(max-width:768px){.sp-quick-actions{right:13px!important;bottom:82px!important}.sp-quick-action-main{width:54px!important;height:54px!important;font-size:29px!important}.sp-quick-action{min-height:40px!important;font-size:12px!important}}
    `;
  ...
  function readLoggedInUsername(){
    ...
    var labels=Array.prototype.slice.call(document.querySelectorAll("span,p,div"));
    for(var i=0;i<labels.length;i++){
      var label=labels[i];
      if((label.textContent||"").trim().toLocaleLowerCase("tr-TR")==="kullanıcı adı:"){
        var sibling=label.nextElementSibling;
        var value=sibling ? (sibling.textContent||"").trim() : "";
    ...
    return /^[A-Za-z0-9._-]{3,50}$/.test(savedUsername) ? savedUsername : "";
  }
  ...
  function openDepositPanel(){
    ...
    var target=candidates.find(function(item){
      ...
      return visible&&(text==="YATIR"||text==="PARA YATIR");
    });
  ...
  function refresh(){ enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); enhanceQuickActions(); }
