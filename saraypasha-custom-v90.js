(function () {
  ...
  function syncMobilePaymentBrand(form){
    ...
    var labels={"113":"Hemen Havale","114":"Hemen Papara","277":"Tether USDT / TRC20"};
    if(!labels[paymentId]){
      if(existing) existing.remove();
      form.removeAttribute("data-sp-payment-brand");
      return;
    }
    var title=Array.prototype.slice.call(form.querySelectorAll("p")).find(function(item){
      return normalizeTurkish(item.textContent||"").indexOf(normalizeTurkish(labels[paymentId]))!==-1;
    });
    if(!title){
      if(existing) existing.remove();
      form.removeAttribute("data-sp-payment-brand");
    ...
    var holder=title.parentElement||form;
    if(!existing){
    ...
    if(existing.getAttribute("src")!==source) existing.setAttribute("src",source);
