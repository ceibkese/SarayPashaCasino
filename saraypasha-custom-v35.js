(function () {
  ...
  function openBonusPageAfterUsernameCapture(){
    ...
    var captureTimer=setInterval(function(){
      attempts+=1;
      var username=readLoggedInUsername(false);
      if(username||attempts>=18){
  ...
  function enhanceAnnouncementBar(){
    ...
    if(currentAnnouncement&&!announcementTemplate) announcementTemplate=currentAnnouncement.cloneNode(true);

    if(!currentAnnouncement&&isHomePage()&&announcementTemplate){
      var header=document.querySelector("header");
      var restored=announcementTemplate.cloneNode(true);
      if(header&&header.parentNode) header.parentNode.insertBefore(restored,header);
      else if(document.body) document.body.insertBefore(restored,document.body.firstChild);
