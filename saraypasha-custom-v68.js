(function () {
  ...
  function isCasinoPage(){
    return /^\/(tr|en)\/casino\/?$/i.test(window.location.pathname);
  }
  ...
  function syncPageMode(){
    ...
    document.body.classList.toggle("sp-casino-page",isCasinoPage());
    document.body.classList.toggle("sp-sports-page",sports);
    if(!home) removeHomeBannerExtras();
    if(!isCasinoPage()) removeCasinoProviderButtons();
  }
  ...
  function removeCasinoProviderButtons(){
    document.querySelectorAll(".sp-casino-provider-buttons").forEach(function(item){item.remove();});
  }

  function enhanceCasinoProviderButtons(){
    if(!isCasinoPage()||!window.matchMedia("(max-width: 768px)").matches){
      removeCasinoProviderButtons();
      return;
    }
    if(document.querySelector(".sp-casino-provider-buttons")) return;
    var module=document.querySelector('[data-mj="game-catalog-module"]');
    if(!module||!module.children||!module.children.length) return;
    var host=module.children[0];
    var language=(window.location.pathname.match(/^\/(tr|en)(?:\/|$)/i)||[])[1]||"tr";
    var nav=document.createElement("nav");
    nav.className="sp-casino-provider-buttons";
    nav.setAttribute("aria-label","Favori oyun sağlayıcıları");
    nav.innerHTML=
      '<a href="/'+language+'/casino/pragmatic-play" aria-label="Pragmatic Play"><img src="https://cdn.jsdelivr.net/gh/ceibkese/SarayPashaCasino@main/pragmatic-play-hemen-oyna.png" alt="Pragmatic Play"></a>'+ 
      '<a href="/'+language+'/casino/egt-digital" aria-label="EGT Digital"><img src="https://cdn.jsdelivr.net/gh/ceibkese/SarayPashaCasino@main/egt-digital-bell-link-hemen-oyna.png" alt="EGT Digital"></a>'+ 
      '<a href="/'+language+'/casino/amusnet" aria-label="Amusnet"><img src="https://cdn.jsdelivr.net/gh/ceibkese/SarayPashaCasino@main/amusnet-egt-hemen-oyna.png" alt="Amusnet"></a>';
  ...
  function refresh(){ syncPageMode(); enhanceBankTransfer(); enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); enhanceQuickActions(); enhanceProviderShortcuts(); enhanceBannerDots(); enhanceCasinoProviderButtons(); enhanceAnnouncementBar(); watchWithdrawalPanel(); watchDepositPanel(); continuePendingAction(); }
  ...
  function destroy(){
    ...
    document.querySelectorAll(".sp-provider-shortcuts,.sp-banner-dots,.sp-casino-provider-buttons,.sp-quick-actions,.sp-copy-toast,.sp-keyboard-page-shield").forEach(function(item){item.remove();});
    document.documentElement.classList.remove("sp-bank-transfer-selected");
  }
