(function () {
  ...
  function hasCompleteInitialHavaleForm(){
    return Array.prototype.slice.call(document.querySelectorAll("form")).some(function(form){
      if(!form.getClientRects||form.getClientRects().length===0) return false;
      var text=normalizeTurkish(form.textContent||"");
      if(text.indexOf("HEMEN HAVALE")===-1||text.indexOf("KILITLI BAKIYE")===-1) return false;
  ...
  function recoverIncompleteInitialHavale(){
    if(initialHavaleRecoveryDone||!isBankTransferWithdrawalRoute()||hasCompleteInitialHavaleForm()) return;
    var selected=getSelectedPaymentIds();
