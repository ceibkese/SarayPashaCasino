(function () {
  ...
  function openDepositPanel(){
    ...
    var target=candidates.find(function(item){
      ...
      var raw=(item.innerText||item.textContent||item.getAttribute("aria-label")||item.getAttribute("title")||"").replace(/\s+/g," ").trim();
      var text=normalizeTurkish(raw);
      return text==="YATIR"||text==="PARA YATIR";
    });
  ...
  function continuePendingAction(){
    ...
    var deposit=Array.prototype.slice.call(document.querySelectorAll("button,a,[role='button']")).find(function(item){
      if(item.closest&&item.closest(".sp-quick-actions")) return false;
      var raw=(item.innerText||item.textContent||item.getAttribute("aria-label")||item.getAttribute("title")||"").replace(/\s+/g," ").trim();
      var text=normalizeTurkish(raw);
      return text==="YATIR"||text==="PARA YATIR";
  ...
  function refresh(){ enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); enhanceQuickActions(); continuePendingAction(); }
