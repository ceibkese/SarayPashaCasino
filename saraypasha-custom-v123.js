(function () {
  ...
  /* Odeme penceresi acilmadan yerel yontem logolarini tarayici onbellegine
     al. Boylece kartlar ve form logosu ayni anda boyanabilir. */
  ["410","113","114","277"].forEach(function(paymentId){
    var paymentImage=new Image();
    paymentImage.src="/api/cmsgateway/api/v1.0/AssetsSite/payments/"+paymentId;
  });
  var trc20Image=new Image();
  trc20Image.src="https://cdn.jsdelivr.net/gh/ceibkese/SarayPashaCasino@main/trc20-form-logo-v2.png?v=2";
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      input.sp-withdrawal-iban-invalid{border-color:#ff6b6b!important;box-shadow:0 0 0 3px rgba(255,74,74,.14)!important}
      html.sp-withdrawal-method-switching body [role="alertdialog"] form{
      ...
      @media(max-width:768px){
        div[data-mj="bottom-nav"] a[data-mj="bottom-nav-item"][href$="/livecasino"]{order:1!important}
        div[data-mj="bottom-nav"] a[data-mj="bottom-nav-item"][href$="/casino"]{order:2!important}
        div[data-mj="bottom-nav"] a[data-mj="bottom-nav-item"][href="/tr/"],div[data-mj="bottom-nav"] a[data-mj="bottom-nav-item"][href="/en/"]{order:3!important}
        div[data-mj="bottom-nav"] a[data-mj="bottom-nav-item"][href^="/tr/sport"],div[data-mj="bottom-nav"] a[data-mj="bottom-nav-item"][href^="/en/sport"]{order:4!important}
        div[data-mj="bottom-nav"] span[data-mj="bottom-nav-item"]{order:5!important}
        div[data-mj="bottom-nav"] a[data-mj="bottom-nav-item"][href^="/tr/sport"]>span:first-child,div[data-mj="bottom-nav"] a[data-mj="bottom-nav-item"][href^="/en/sport"]>span:first-child{-webkit-mask:none!important;mask:none!important;background:url("https://cdn.jsdelivr.net/gh/ceibkese/SarayPashaCasino@main/sporbut.png") center/24px 24px no-repeat!important;background-color:transparent!important;opacity:1!important;visibility:visible!important;width:26px!important;height:26px!important;min-width:26px!important;display:inline-block!important}
        ...
        body.sp-withdrawal-amount-focused .sp-quick-actions{
          opacity:0!important;
          visibility:hidden!important;
          pointer-events:none!important
        }
        body.sp-withdrawal-amount-focused>#root{
          visibility:hidden!important;
          pointer-events:none!important
        }
        html.sp-withdrawal-amount-focused,
        html.sp-withdrawal-amount-focused body{
          height:100%!important;
        ...
        body.sp-withdrawal-amount-focused .sp-withdrawal-amount-overlay{
        ...
        body.sp-withdrawal-amount-focused .sp-withdrawal-amount-scroller{
        ...
        body.sp-withdrawal-amount-focused .sp-withdrawal-amount-scroller::-webkit-scrollbar{
          display:none!important;
          width:0!important;
          height:0!important
        }
        body.sp-withdrawal-amount-focused .sp-withdrawal-amount-positioner{
          transform:translate3d(0,calc(-1 * var(--sp-withdrawal-keyboard-shift,0px)),0)!important;
  ...
  function clearMobilePaymentBrands(scope){
    var root=scope&&scope.querySelectorAll?scope:document;
    Array.prototype.slice.call(root.querySelectorAll(".sp-mobile-payment-brand")).forEach(function(item){item.remove();});
    Array.prototype.slice.call(root.querySelectorAll("[data-sp-payment-brand]")).forEach(function(item){item.removeAttribute("data-sp-payment-brand");});
    if(root.matches&&root.matches("[data-sp-payment-brand]")) root.removeAttribute("data-sp-payment-brand");
  }

  function syncMobilePaymentBrand(form){
    if(!form) return;
    var existing=form.querySelector(".sp-mobile-payment-brand");
    var mobile=window.matchMedia&&window.matchMedia("(max-width: 768px)").matches;
    var selected=Array.prototype.slice.call(document.querySelectorAll('img[alt="payment-logo"]')).filter(isPaymentLogoSelected);
    if(!mobile||selected.length!==1){
      if(existing) existing.remove();
      form.removeAttribute("data-sp-payment-brand");
      return;
    }
    var source=String(selected[0].currentSrc||selected[0].getAttribute("src")||selected[0].src||"");
    var paymentId=(source.match(/\/payments\/(\d+)/)||[])[1]||"";
    if(["113","114","277","410"].indexOf(paymentId)===-1){
      if(existing) existing.remove();
      form.removeAttribute("data-sp-payment-brand");
      return;
    }
    /* Yontem adlari Lynon panelinde degisebildigi icin baslik metnine baglanma.
       Tether formunda Lynon'un kucuk yatay kart gorseli yerine SarayPasha'nin
       seffaf TRC20 form logosunu kullan. */
    var brandSource=paymentId==="277"
      ? "https://cdn.jsdelivr.net/gh/ceibkese/SarayPashaCasino@main/trc20-form-logo-v2.png?v=2"
      : source;
    var holder=form.firstElementChild||form;
    if(!existing){
      existing=document.createElement("img");
      existing.className="sp-mobile-payment-brand";
      existing.setAttribute("aria-hidden","true");
      existing.alt="";
      existing.draggable=false;
      holder.appendChild(existing);
    }else if(existing.parentElement!==holder){
      holder.appendChild(existing);
    }
    if(existing.getAttribute("src")!==brandSource) existing.setAttribute("src",brandSource);
    form.setAttribute("data-sp-payment-brand",paymentId);
  }

  var paymentBrandFrame=0;
  function queueImmediatePaymentBrandSync(){
    if(!window.matchMedia||!window.matchMedia("(max-width: 768px)").matches) return;
    if(!isBankTransferDepositRoute()&&!isBankTransferWithdrawalRoute()) return;
    if(paymentBrandFrame) return;
    paymentBrandFrame=requestAnimationFrame(function(){
      paymentBrandFrame=0;
      Array.prototype.slice.call(document.querySelectorAll("form")).forEach(function(form){
        if(!form.getClientRects||form.getClientRects().length===0) return;
        if(!form.querySelector('input[placeholder="Tutarı Girin"],input[placeholder*="Tutar"]')) return;
        syncMobilePaymentBrand(form);
      });
      if(isBankTransferWithdrawalRoute()) enhanceWithdrawalFrames();
    });
  }

  function syncPaymentPlaceholderDescription(form){
    if(!form) return;
    Array.prototype.slice.call(form.querySelectorAll(".sp-payment-placeholder-description")).forEach(function(item){
      item.classList.remove("sp-payment-placeholder-description");
    });
    var selected=Array.prototype.slice.call(document.querySelectorAll('img[alt="payment-logo"]')).filter(isPaymentLogoSelected);
    if(selected.length!==1) return;
    var source=String(selected[0].currentSrc||selected[0].getAttribute("src")||selected[0].src||"");
    var paymentId=(source.match(/\/payments\/(\d+)/)||[])[1]||"";
    if(["113","114"].indexOf(paymentId)===-1) return;
    var holder=form.firstElementChild||form;
    Array.prototype.slice.call(form.querySelectorAll("p")).forEach(function(paragraph){
      var label=normalizeTurkish(paragraph.textContent||"").replace(/\s+/g," ").trim();
      if(!/^DESC(?:RIPTION)?(?:\s|$)/.test(label)) return;
      var block=paragraph;
      while(block.parentElement&&block.parentElement!==holder) block=block.parentElement;
      block.classList.add("sp-payment-placeholder-description");
    });
  }
      ...
      loadBankAccount().then(function(account){
        ...
        if(panel.dataset.spAccountKey!==accountKey){
          ...
          if(!account.active){
            ...
            panel.innerHTML='<div class="sp-bank-limits"><span>İŞLEM LİMİTLERİ</span><div><strong>Minimum Yatırım</strong><b></b></div><div><strong>Maksimum Yatırım</strong><b></b></div><div class="sp-bank-fast-logo"><img src="https://cdn.jsdelivr.net/gh/ceibkese/SarayPashaCasino@main/fast-logo-v1.png?v=2" alt="FAST"></div></div>'+
        ...
        form.dataset.spBankLoading="0";
        /* Panel hazir oldugunda, MutationObserver zamanlamasina bagli
           kalmadan formu tek karede gorunur duruma getir. */
        if(canApplyBankTransferResult(form,bankRequestGeneration)){
          form.classList.add("sp-deposit-method-frame");
          document.documentElement.classList.remove("sp-bank-transfer-preparing");
        }
      });
    });
  ...
  function enhanceWithdrawalFrames(){
    ...
    var found=false;
    var activePaymentId=getSelectedPaymentIds().filter(function(id){return ["113","114","277","410"].indexOf(id)!==-1;})[0]||"";
    forms.forEach(function(form){
      ...
      syncMobilePaymentBrand(form);
      syncPaymentPlaceholderDescription(form);
      if(activePaymentId&&!isWithdrawalMethodFormReady(activePaymentId,form)) return;
      found=true;
    });
    if(found) document.documentElement.classList.remove("sp-withdrawal-preparing");
  }

  function isWithdrawalMethodFormReady(paymentId,targetForm){
    if(["113","114","277","410"].indexOf(paymentId)===-1) return true;
    if(getSelectedPaymentIds().indexOf(paymentId)===-1) return false;
    var forms=targetForm?[targetForm]:Array.prototype.slice.call(document.querySelectorAll("form"));
    return forms.some(function(form){
      if(!form||!form.getClientRects||form.getClientRects().length===0) return false;
      var amount=form.querySelector('input[placeholder="Tutarı Girin"],input[placeholder*="Tutar"]');
      var text=normalizeTurkish(form.textContent||"");
      if(!amount||text.indexOf("KILITLI BAKIYE")===-1) return false;
      /* Hemen Havale limitleri bazı açılışlarda formun dışında kalıyor.
         Bu yöntemde temel alanlar hazırsa görünürlüğü limit metnine bağlama. */
      if(paymentId!=="113"&&(text.indexOf("MIN")===-1||text.indexOf("MAKS")===-1)) return false;
      if(["113","410"].indexOf(paymentId)!==-1){
        var iban=findWithdrawalIbanInput(form);
        var bankSelect=form.querySelector('input[id^="react-select-"]');
        if(!iban||!bankSelect||text.indexOf("IBAN")===-1||text.indexOf("BANKA")===-1) return false;
      }
      /* Hemen Havale formunun gorunmesini yalnizca dekoratif logonun yuklenme
         sinyaline baglama. iOS bazen gorseli ekranda gosterdigi halde
         naturalWidth bilgisini gec/eksik bildiriyor ve form kapali kalabiliyor. */
      if(paymentId==="113") return true;
      if(window.matchMedia&&window.matchMedia("(max-width: 768px)").matches){
        var brand=form.querySelector("img.sp-mobile-payment-brand");
        if(form.getAttribute("data-sp-payment-brand")!==paymentId||!brand||!brand.complete||!brand.naturalWidth) return false;
      }
      return true;
    });
  }

  function isStrictIbanWithdrawalMethodSelected(){
    var selected=getSelectedPaymentIds();
    return selected.length===1&&["113","410"].indexOf(selected[0])!==-1;
  }

  function isStrictWithdrawalFormReady(paymentId){
    return isWithdrawalMethodFormReady(paymentId);
  }

  function beginWithdrawalMethodSwitch(event){
    if(!isBankTransferWithdrawalRoute()) return;
    ...
    if(!logo||isPaymentLogoSelected(logo)) return;
    var source=String(logo.getAttribute("src")||logo.src||"");
    var paymentId=(source.match(/\/payments\/(\d+)/)||[])[1]||"";
    if(["113","114","277","410"].indexOf(paymentId)===-1) return;
    var generation=++paymentSwitchGeneration;
    document.documentElement.classList.add("sp-withdrawal-method-switching");
    [40,90,160,280,460,750,1100,1600,2300,3000].forEach(function(delay){
      setTimeout(function(){
        if(generation!==paymentSwitchGeneration) return;
        refresh();
        if(isStrictWithdrawalFormReady(paymentId)){
          document.documentElement.classList.remove("sp-withdrawal-method-switching");
        }
      },delay);
    });
    setTimeout(function(){
      if(generation===paymentSwitchGeneration){
        document.documentElement.classList.remove("sp-withdrawal-method-switching");
        refresh();
      }
    },6500);
  ...
  function normalizeWithdrawalIban(value){
    return String(value||"").toUpperCase().replace(/\s+/g,"");
  }

  function findWithdrawalIbanInput(form){
    if(!form) return null;
    var labels=Array.prototype.slice.call(form.querySelectorAll("label"));
    for(var index=0;index<labels.length;index++){
      var label=labels[index];
      if(normalizeTurkish(label.textContent||"").indexOf("IBAN")===-1) continue;
      var targetId=label.getAttribute("for");
      var target=targetId?document.getElementById(targetId):null;
      if(target&&form.contains(target)&&target.tagName==="INPUT") return target;
      var nested=label.querySelector("input");
      if(nested) return nested;
    }
    return form.querySelector('input[name="inputs.0.value"],input[name="inputs.1.value"]');
  }

  function enhanceWithdrawalIbanValidation(){
    if(!isBankTransferWithdrawalRoute()||!isStrictIbanWithdrawalMethodSelected()) return;
    Array.prototype.slice.call(document.querySelectorAll("form")).forEach(function(form){
      if(!form.getClientRects||form.getClientRects().length===0) return;
      var amount=form.querySelector('input[placeholder="Tutarı Girin"],input[placeholder*="Tutar"]');
      var iban=findWithdrawalIbanInput(form);
      ...
      iban.setAttribute("autocapitalize","characters");
      iban.setAttribute("maxlength","32");
      iban.setAttribute("aria-required","true");

      function valid(input){
        return /^TR\d{24}$/.test(normalizeWithdrawalIban(input&&input.value));
      }
      function clearInvalid(input){
        if(!input||!valid(input)) return;
        input.classList.remove("sp-withdrawal-iban-invalid");
        input.removeAttribute("aria-invalid");
      }
      function check(event){
        if(!isStrictIbanWithdrawalMethodSelected()) return true;
        var current=findWithdrawalIbanInput(form);
        if(current&&valid(current)){
          clearInvalid(current);
          return true;
        }
        if(event){
        ...
        if(current){
          current.classList.add("sp-withdrawal-iban-invalid");
          current.setAttribute("aria-invalid","true");
          current.focus();
        }
        showToast("Lütfen TR ile başlayan 26 karakterli geçerli IBAN numaranızı giriniz.");
        return false;
      }

      if(iban.dataset.spWithdrawalIbanInputReady!=="1"){
        iban.dataset.spWithdrawalIbanInputReady="1";
        iban.addEventListener("input",function(){clearInvalid(iban);});
      }
      if(form.dataset.spWithdrawalIbanValidationReady!=="1"){
        form.dataset.spWithdrawalIbanValidationReady="1";
        ...
        form.addEventListener("click",function(event){
          ...
          if(button&&form.contains(button)) check(event);
        },true);
      }
    });
  }
  ...
  function enhanceDepositFrames(){
    ...
    if(!isBankTransferDepositRoute()){
      ...
      clearMobilePaymentBrands(document);
      return;
    }
    forms.forEach(function(form){
      var amount=form.querySelector('input[placeholder="Tutarı Girin"],input[placeholder*="Tutar"]');
      if(!amount||form.getClientRects().length===0) return;
      /* Bank Transfer'in Lynon'a ait eski formunu masaustu ve mobilde,
         ozel banka paneli tamamen hazir olana kadar gosterme. */
      if(isBankTransferSelected()&&form.dataset.spBankLoading!=="0"){
        form.classList.remove("sp-deposit-method-frame");
        clearMobilePaymentBrands(form);
        return;
      }
      form.classList.add("sp-deposit-method-frame");
      syncMobilePaymentBrand(form);
  ...
  function beginNonBankPaymentSwitch(event){
    ...
    if(source.indexOf("/payments/275")!==-1){
      ...
      var bankGeneration=++paymentSwitchGeneration;
      nonBankMethodSwitchUntil=0;
      document.documentElement.classList.remove("sp-payment-method-switching");
      document.documentElement.classList.add("sp-bank-transfer-preparing");
      setTimeout(refresh,0);
      setTimeout(refresh,80);
      /* CDN veya banka servisi cevap vermezse form sonsuza kadar kapali kalmaz. */
      setTimeout(function(){
        if(bankGeneration===paymentSwitchGeneration) document.documentElement.classList.remove("sp-bank-transfer-preparing");
      },2500);
      return;
    }
  ...
  var withdrawalAmountFocusStartedAt=0;
  var withdrawalAmountKeyboardSeen=false;
  var withdrawalAmountViewportBaseline=0;
  ...
  function stopDepositScrollLock(){
    ...
    withdrawalAmountFocusStartedAt=0;
    withdrawalAmountKeyboardSeen=false;
    withdrawalAmountViewportBaseline=0;
    document.documentElement.classList.remove("sp-deposit-scroll-locked");
    document.documentElement.classList.remove("sp-withdrawal-amount-focused");
    document.body.classList.remove("sp-deposit-scroll-locked");
    document.body.classList.remove("sp-trc20-input-focused");
    document.body.classList.remove("sp-withdrawal-amount-focused");
    ...
    Array.prototype.slice.call(document.querySelectorAll(".sp-withdrawal-amount-overlay")).forEach(function(item){
      item.classList.remove("sp-withdrawal-amount-overlay");
    });
    Array.prototype.slice.call(document.querySelectorAll(".sp-withdrawal-amount-scroller")).forEach(function(item){
      item.classList.remove("sp-withdrawal-amount-scroller");
    });
    Array.prototype.slice.call(document.querySelectorAll(".sp-withdrawal-amount-positioner")).forEach(function(item){
      item.classList.remove("sp-withdrawal-amount-positioner");
      item.style.removeProperty("--sp-withdrawal-keyboard-shift");
    });
  ...
  if(window.visualViewport&&!window.__spDepositViewportLockReady){
    ...
    var onDepositViewportChange=function(){setTimeout(function(){
      ...
      if(document.documentElement.classList.contains("sp-withdrawal-amount-focused")){
        var amountInput=document.activeElement;
        var viewportHeight=window.visualViewport.height||0;
        if(withdrawalAmountViewportBaseline&&viewportHeight<withdrawalAmountViewportBaseline-90){
          withdrawalAmountKeyboardSeen=true;
        }
        if(withdrawalAmountKeyboardSeen&&
           viewportHeight>=withdrawalAmountViewportBaseline-50&&
           Date.now()-withdrawalAmountFocusStartedAt>350){
          if(amountInput&&typeof amountInput.blur==="function") amountInput.blur();
          stopDepositScrollLock();
          return;
        }
        if(isStrictWithdrawalAmountInput(amountInput)){
          var amountOverlay=findFixedOverlay(amountInput);
          markWithdrawalAmountScrollers(amountInput,amountOverlay);
          [0,60,150,280].forEach(function(delay){
            setTimeout(function(){revealWithdrawalAmountInput(amountInput);},delay);
          });
        }
      }
    },0);};
  ...
  var blockWithdrawalAmountPan=function(event){
    if(document.documentElement.classList.contains("sp-withdrawal-amount-focused")) event.preventDefault();
  };
  listenGlobal(document,"touchmove",blockWithdrawalAmountPan,{capture:true,passive:false});
  listenGlobal(document,"wheel",blockWithdrawalAmountPan,{capture:true,passive:false});

  function isStrictWithdrawalAmountInput(input){
    if(!input||window.innerWidth>768||!isBankTransferWithdrawalRoute()||!isStrictIbanWithdrawalMethodSelected()) return false;
    var placeholder=normalizeTurkish(input.getAttribute("placeholder")||"");
    return placeholder.indexOf("TUTAR")!==-1&&!!input.closest("form");
  }

  function positionWithdrawalAmountForKeyboard(input){
    if(!input||document.activeElement!==input) return false;
    var current=input.parentElement;
    var fallback=null;
    var positioner=null;
    while(current&&current!==document.body){
      if(current.classList.contains("sp-withdrawal-amount-scroller")){
        fallback=current;
        if(current.scrollHeight>current.clientHeight+1) positioner=current;
      }
      current=current.parentElement;
    }
    positioner=positioner||fallback;
    ...
    positioner.classList.add("sp-withdrawal-amount-positioner");
    var viewport=window.visualViewport;
    var visibleHeight=(viewport&&viewport.height)||(window.innerHeight||700);
    var visibleTop=(viewport&&viewport.offsetTop)||0;
    var targetTop=visibleTop+Math.min(270,Math.max(230,visibleHeight*0.55));
    var currentShift=parseFloat(positioner.style.getPropertyValue("--sp-withdrawal-keyboard-shift"))||0;
    var inputTop=input.getBoundingClientRect().top;
    var nextShift=Math.max(0,Math.min(360,currentShift+(inputTop-targetTop)));
    positioner.style.setProperty("--sp-withdrawal-keyboard-shift",nextShift.toFixed(1)+"px");
    return true;
  }

  function revealWithdrawalAmountInput(input){
    if(!input||document.activeElement!==input) return;
    if(positionWithdrawalAmountForKeyboard(input)) return;
    if(input.scrollIntoView) input.scrollIntoView({block:"center",inline:"nearest",behavior:"auto"});
  }

  function markWithdrawalAmountScrollers(input,overlay){
    var current=input&&input.parentElement;
    while(current&&current!==document.body){
      var overflowY=window.getComputedStyle(current).overflowY||"";
      if(/auto|scroll/i.test(overflowY)) current.classList.add("sp-withdrawal-amount-scroller");
      if(current===overlay) break;
      current=current.parentElement;
    }
  }
  ...
  function bindKeyboard(input, overlay) {
    ...
    input.addEventListener("focus",function(){
      /* Portal, mobil Safari'de ilk cizimden sonra fixed hale gelebilir.
         Bu nedenle ilk baglamada tutulan deger yerine odak aninda yeniden bul. */
      var focusOverlay=findFixedOverlay(input)||overlay;
      if(isStrictWithdrawalAmountInput(input)){
        /* IBAN/banka alanindan tutara ilk geciste kalan mobil kaydirma kilidini
           kaldir. Ana sayfayi koruyucu katmanla kapali tutarken tutari ortaya al. */
        stopDepositScrollLock();
        withdrawalAmountFocusStartedAt=Date.now();
        withdrawalAmountKeyboardSeen=false;
        withdrawalAmountViewportBaseline=Math.max(
          window.innerHeight||0,
          (window.visualViewport&&window.visualViewport.height)||0
        );
        document.documentElement.classList.add("sp-withdrawal-amount-focused");
        document.body.classList.add("sp-withdrawal-amount-focused");
        if(focusOverlay){
          focusOverlay.classList.add("sp-withdrawal-amount-overlay");
          ensureKeyboardShield(focusOverlay);
        }
        markWithdrawalAmountScrollers(input,focusOverlay);
        [0,70,150,260,420,650,900,1200].forEach(function(delay){
          setTimeout(function(){
            markWithdrawalAmountScrollers(input,focusOverlay);
            revealWithdrawalAmountInput(input);
          },delay);
        });
        return;
      }
      document.documentElement.classList.remove("sp-withdrawal-amount-focused");
      document.body.classList.remove("sp-withdrawal-amount-focused");
      if(focusOverlay) focusOverlay.classList.remove("sp-withdrawal-amount-overlay");
      startDepositScrollLock(input,focusOverlay);
      ...
      if(focusOverlay) {
        var coverHeight=Math.max(
          window.innerHeight||0,
          document.documentElement.clientHeight||0,
          Math.round(focusOverlay.getBoundingClientRect().height)||0
        );
        focusOverlay.classList.add("sp-trc20-modal-overlay");
        focusOverlay.style.setProperty("--sp-deposit-cover-height",coverHeight+"px");
        ensureKeyboardShield(focusOverlay);
      }
      var surface=focusOverlay && focusOverlay.querySelector(".modal"); if(surface) surface.classList.add("sp-trc20-modal-surface");
    });
  ...
  function enhanceBonusRequestPage(){
    ...
    frame.src="https://saraypasha.com/bonus-embed.php?casino_bonus_only=1";
  ...
  function watchWithdrawalPanel(){
    ...
    if(withdrawalPanelWasOpen&&!open){
      /* Klavye acikken panel kapatilirsa mobil kilit siniflari ana sayfaya
         tasinmasin. Menuleri ve ana icerigi hemen normal durumuna dondur. */
      stopDepositScrollLock();
  ...
  function refresh(){ syncPageMode(); syncSportsIframeHeight(); enhanceDepositFrames(); enhanceBankTransfer(); enhanceWithdrawalFrames(); enhanceWithdrawalIbanValidation(); enhanceBankWithdrawal(); enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); enhanceQuickActions(); enhanceProviderShortcuts(); enhanceBannerDots(); enhanceAnnouncementBar(); watchWithdrawalPanel(); watchDepositPanel(); continuePendingAction(); }

  var withdrawalOpenRepairTimers=[];
  var initialHavaleRecoveryDone=false;
  function clearWithdrawalOpenRepairTimers(){
    withdrawalOpenRepairTimers.forEach(function(timerId){clearTimeout(timerId);});
    withdrawalOpenRepairTimers=[];
  }

  function repairWithdrawalOpening(){
    if(!isBankTransferWithdrawalRoute()) return;
    /* Lynon ilk acilista Hemen Havale formunu bir kez kaldirip yeniden
       olusturabiliyor. Yeni olusan forma siniflari dogrudan tekrar uygula. */
    enhanceWithdrawalFrames();
    enhanceWithdrawalIbanValidation();
    enhanceBankWithdrawal();
    enhanceWithdrawalFrames();
  }

  function findInitialHavaleForm(){
    return Array.prototype.slice.call(document.querySelectorAll("form")).find(function(form){
      if(!form.getClientRects||form.getClientRects().length===0) return false;
      var text=normalizeTurkish(form.textContent||"");
      return text.indexOf("HEMEN HAVALE")!==-1&&text.indexOf("KILITLI BAKIYE")!==-1&&
        !!form.querySelector('input[placeholder="Tutarı Girin"],input[placeholder*="Tutar"]');
    })||null;
  }

  function hasCompleteInitialHavaleForm(){
    var form=findInitialHavaleForm();
    if(!form) return false;
    var text=normalizeTurkish(form.textContent||"");
    return !!findWithdrawalIbanInput(form)&&!!form.querySelector('input[id^="react-select-"]')&&
      text.indexOf("IBAN")!==-1&&text.indexOf("BANKA")!==-1;
  }

  function recoverIncompleteInitialHavale(){
    if(initialHavaleRecoveryDone||!isBankTransferWithdrawalRoute()||
      document.documentElement.classList.contains("sp-withdrawal-method-switching")||
      !findInitialHavaleForm()||hasCompleteInitialHavaleForm()) return;
    var selected=getSelectedPaymentIds();
    if(selected.length!==1||selected[0]!=="113") return;
    var havaleLogo=document.querySelector('img[alt="payment-logo"][src*="/payments/113"]');
    var alternateLogo=document.querySelector('img[alt="payment-logo"][src*="/payments/410"]')||
      document.querySelector('img[alt="payment-logo"][src*="/payments/114"]');
    if(!havaleLogo||!alternateLogo) return;
    initialHavaleRecoveryDone=true;
    document.documentElement.classList.add("sp-withdrawal-preparing");
    /* Lynon bazen ilk varsayilan 113 secimini yalnizca Tutar alaniyla kuruyor.
       Mevcut kartlari bir kez yeniden secmek React formunu eksiksiz kuruyor. */
    alternateLogo.click();
    withdrawalOpenRepairTimers.push(setTimeout(function(){
      if(!isBankTransferWithdrawalRoute()||!document.contains(havaleLogo)) return;
      havaleLogo.click();
      [40,140,320,650,1050].forEach(function(delay){
        withdrawalOpenRepairTimers.push(setTimeout(repairWithdrawalOpening,delay));
      });
    },80));
  }

  function scheduleWithdrawalOpenRepair(){
    clearWithdrawalOpenRepairTimers();
    if(!isBankTransferWithdrawalRoute()) return;
    initialHavaleRecoveryDone=false;
    [0,70,160,300,520,800,1150,1550,2050,2700,3500,4500].forEach(function(delay){
      withdrawalOpenRepairTimers.push(setTimeout(repairWithdrawalOpening,delay));
    });
    withdrawalOpenRepairTimers.push(setTimeout(recoverIncompleteInitialHavale,900));
    /* Herhangi bir cihazda hazirlik sinyali kaybolursa form sonsuza kadar
       kapali kalmasin. Bu yalnizca ilk acilis kilidini serbest birakir. */
    withdrawalOpenRepairTimers.push(setTimeout(function(){
      if(!isBankTransferWithdrawalRoute()) return;
      document.documentElement.classList.remove("sp-withdrawal-preparing");
      enhanceWithdrawalFrames();
  ...
  listenGlobal(window,"resize",function(){syncSportsIframeHeight();enhanceDepositFrames();enhanceWithdrawalFrames();});
  listenGlobal(window,"orientationchange",function(){setTimeout(function(){syncSportsIframeHeight();enhanceDepositFrames();enhanceWithdrawalFrames();},160);});
  listenGlobal(document,"pointerdown",beginWithdrawalMethodSwitch,true);
  ...
  function destroy(){
    ...
    if(paymentBrandObserver){paymentBrandObserver.disconnect();paymentBrandObserver=null;}
    if(paymentBrandFrame){cancelAnimationFrame(paymentBrandFrame);paymentBrandFrame=0;}
    ...
    document.querySelectorAll(".sp-provider-shortcuts,.sp-banner-dots,.sp-quick-actions,.sp-copy-toast,.sp-keyboard-page-shield,.sp-mobile-payment-brand").forEach(function(item){item.remove();});
    document.documentElement.classList.remove("sp-bank-transfer-selected");
    document.documentElement.classList.remove("sp-payment-method-switching");
    document.documentElement.classList.remove("sp-withdrawal-method-switching");
    document.documentElement.classList.remove("sp-bank-transfer-preparing");
  ...
  addStyles(); refresh();
  if(isBankTransferWithdrawalRoute()) scheduleWithdrawalOpenRepair();
  ...
  paymentBrandObserver=new MutationObserver(queueImmediatePaymentBrandSync);
  paymentBrandObserver.observe(document.documentElement,{childList:true,subtree:true});
  queueImmediatePaymentBrandSync();
  ...
  maintenanceTimer=setInterval(function(){
    ...
    /* React'in ilk Hemen Havale formunu eksik kurduğu anı zamanlayıcıdan
       bağımsız yakala. Kullanıcının yöntem geçişi sırasında devreye girmez. */
    recoverIncompleteInitialHavale();
  },500);
