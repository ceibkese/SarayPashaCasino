(function () {
  ...
  function revealWithdrawalAmountInput(input){
    if(!input||document.activeElement!==input||!input.scrollIntoView) return;
    var viewport=window.visualViewport;
    if(viewport&&viewport.height<(window.innerHeight||viewport.height)-80){
      var rect=input.getBoundingClientRect();
      var visibleTop=viewport.offsetTop||0;
      var visibleBottom=visibleTop+viewport.height;
      var safeTop=visibleTop+70;
      var safeBottom=visibleBottom-28;
      var delta=0;
      ...
      if(Math.abs(delta)>1){
        ...
        var scroller=null;
        while(current&&current!==document.body){
          if(current.classList.contains("sp-withdrawal-amount-scroller")&&current.scrollHeight>current.clientHeight+1) scroller=current;
          current=current.parentElement;
        }
        if(scroller){
