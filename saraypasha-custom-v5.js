(function () {
  ...
  function bindKeyboard(input, overlay) {
    ...
    input.addEventListener("blur",function(){ setTimeout(function(){ if(!document.activeElement || !document.activeElement.closest(".sp-trc20-form-ready")) document.body.classList.remove("sp-trc20-input-focused"); },180); });
  }
  ...
  addStyles(); enhance();
  var timer=0; new MutationObserver(function(){ clearTimeout(timer); timer=setTimeout(enhance,100); }).observe(document.documentElement,{childList:true,subtree:true});
  setTimeout(enhance,700); setTimeout(enhance,1800);
