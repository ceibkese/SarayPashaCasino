(function () {
  ...
  function enhanceEgtBellLinkGrid(){
    var sections=Array.prototype.slice.call(document.querySelectorAll('section[data-mj="widget-game-slider"]'));
    sections.forEach(function(section){
      if(!section.querySelector('[data-mj="widget-game-slider-header"] a[href*="provider=egt-digital" i]')) return;
      var list=section.querySelector('[data-mj="widget-game-slider-list"]');
      if(!list) return;
      section.classList.add("sp-egt-bell-grid");

      Array.prototype.slice.call(list.querySelectorAll('img[src*="VerticalGameImages"]')).forEach(function(img){
        var horizontal=img.getAttribute("src").replace("VerticalGameImages","HorizontalGameImages");
        if(img.getAttribute("src")!==horizontal) img.setAttribute("src",horizontal);
      });

      if(list.dataset.spEgtScrollReady==="1") return;
      list.dataset.spEgtScrollReady="1";
      var header=section.querySelector('[data-mj="widget-game-slider-header"]');
      if(!header) return;
      var left=header.querySelector('button[aria-label="arrow_left"]');
      var right=header.querySelector('button[aria-label="arrow_right"]');
      function move(direction,event){
        if(window.innerWidth>768) return;
        event.preventDefault();
        event.stopPropagation();
        if(event.stopImmediatePropagation) event.stopImmediatePropagation();
        list.scrollBy({left:direction*Math.max(176,list.clientWidth*.92),behavior:"smooth"});
      }
      if(left) left.addEventListener("click",function(event){move(-1,event);},true);
      if(right) right.addEventListener("click",function(event){move(1,event);},true);
    });
  }

  function refresh(){ enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); enhanceQuickActions(); enhanceEgtBellLinkGrid(); continuePendingAction(); }
