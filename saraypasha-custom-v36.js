(function () {
  ...
  function enhanceAnnouncementBar(){
    var currentAnnouncement=document.querySelector('[data-mj="announcement"]');
    var line=currentAnnouncement&&currentAnnouncement.querySelector("p");
    var hasAnnouncementText=!!(line&&(line.textContent||"").trim());
    var headerNav=document.querySelector('[data-mj="header-nav"]');
    var headerShell=headerNav&&headerNav.parentElement;
    var currentHost=currentAnnouncement&&currentAnnouncement.parentElement;
    var currentHostIsCorrect=!!(headerShell&&currentHost&&currentHost.parentElement===headerShell);

    if(currentAnnouncement&&hasAnnouncementText&&currentHostIsCorrect){
      announcementTemplate=currentAnnouncement.cloneNode(true);
      try{
        sessionStorage.setItem("spAnnouncementTemplate",currentAnnouncement.outerHTML);
        sessionStorage.setItem("spAnnouncementHostClass",currentHost.className||"");
      }catch(e){}
    }

    if(!announcementTemplate){
      try{
        var savedAnnouncement=sessionStorage.getItem("spAnnouncementTemplate")||"";
        if(savedAnnouncement){
          var templateBox=document.createElement("div");
          templateBox.innerHTML=savedAnnouncement;
          announcementTemplate=templateBox.firstElementChild;
        }
      }catch(e){}
    }

    if(isHomePage()&&(!currentAnnouncement||!hasAnnouncementText||!currentHostIsCorrect)&&announcementTemplate){
      var restored=announcementTemplate.cloneNode(true);
      if(currentAnnouncement&&currentAnnouncement.parentNode) currentAnnouncement.parentNode.removeChild(currentAnnouncement);
      if(headerShell){
        var hostClass="";
        try{hostClass=sessionStorage.getItem("spAnnouncementHostClass")||"";}catch(e){}
        var restoredHost=Array.prototype.slice.call(headerShell.children).find(function(item){return hostClass&&item.className===hostClass;});
        if(!restoredHost){
          restoredHost=document.createElement("div");
          restoredHost.className=hostClass;
          headerShell.insertBefore(restoredHost,headerShell.firstChild);
        }
        restoredHost.textContent="";
        restoredHost.appendChild(restored);
      }else if(document.body){
        document.body.insertBefore(restored,document.body.firstChild);
      }
      currentAnnouncement=restored;
      line=restored.querySelector("p");
    }

    if(!line) return;
    line.classList.add("sp-announcement-line");
    if(line.dataset.spIconCleaned==="1") return;
    ...
    line.dataset.spIconCleaned="1";
  }

  function refresh(){ syncPageMode(); enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); enhanceQuickActions(); enhanceProviderShortcuts(); enhanceBannerDots(); enhanceAnnouncementBar(); continuePendingAction(); }
  ...
  setInterval(function(){
    syncPageMode();
    if(isHomePage()) enhanceAnnouncementBar();
  },500);
