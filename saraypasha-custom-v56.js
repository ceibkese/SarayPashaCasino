(function () {
  ...
  function placeBankPortal(form,panel){
    ...
    if(!bankPositionEventsBound){
      ...
      document.addEventListener("scroll",function(){placeBankPortal(activeBankForm,activeBankPanel);},true);
    }
  ...
  function enhanceBankTransfer(){
    ...
    Array.prototype.slice.call(document.querySelectorAll("form")).forEach(function(form){
      ...
      loadBankAccount().then(function(account){
        ...
        var legacyPanel=form.querySelector(".sp-bank-account-panel");
        ...
        if(!account.active){
          ...
          panel.innerHTML='<div class="sp-bank-limits"><span>İŞLEM LİMİTLERİ</span><div><strong>Minimum Yatırım</strong><b>3.000 TRY</b></div><div><strong>Maksimum Yatırım</strong><b>1.000.000 TRY</b></div></div>'+
        ...
        if(!document.contains(form)) return;
        var legacyPanel=form.querySelector(".sp-bank-account-panel");if(legacyPanel) legacyPanel.remove();
        var panel=getBankPortal();
