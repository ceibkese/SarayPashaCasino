  ...
  function enhanceBankWithdrawal(){
    ...
    Array.prototype.slice.call(document.querySelectorAll("form")).forEach(function(form){
      ...
      if(!form.querySelector(".sp-bank-withdrawal-bank-picker")){
        ...
        select.setAttribute("aria-label","Banka");
        var placeholder=document.createElement("option");
        placeholder.value="";
        placeholder.textContent="SEÇİNİZ";
        placeholder.disabled=true;
        placeholder.selected=!String(bank.value||"").trim();
        select.appendChild(placeholder);
        banks.forEach(function(name){
          var option=document.createElement("option");
          option.value=name;
          option.textContent=name;
          if(String(bank.value||"").trim()===name) option.selected=true;
          select.appendChild(option);
        });
        select.addEventListener("change",function(){setNativeInputValue(bank,select.value);});
