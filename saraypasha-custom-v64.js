(function () {
  ...
  function enhanceAnnouncementBar(){
    if(!window.matchMedia("(max-width: 768px)").matches){
      ...
      document.documentElement.classList.remove("sp-independent-announcement-ready");
      return;
    }
    ...
    try{
      if(nativeText){
        var sourceLine=nativeLine.cloneNode(true);
        var sourceWalker=document.createTreeWalker(sourceLine,NodeFilter.SHOW_TEXT);
        var sourceTextNode=sourceWalker.nextNode();
        if(sourceTextNode) sourceTextNode.nodeValue=sourceTextNode.nodeValue.replace(/^\s*(?:📢|📣|🔊)\s*/,"");
        lineHtml=sourceLine.innerHTML;
      }
      if(!lineHtml) lineHtml=sessionStorage.getItem("spAnnouncementLineHtml")||"";
      if(!lineHtml){
    ...
    /* Mobil duyuru React agacinin disinda, body altinda kalici bir katmandir.
       Spor gecisi platform basligini yeniden kursa bile bu dugum silinmez. */
    var custom=document.querySelector(".sp-stable-announcement");
    if(!custom||custom.parentElement!==document.body){
    ...
    document.documentElement.classList.add("sp-independent-announcement-ready");

    var home=/^\/(tr|en)\/?$/i.test(window.location.pathname);
    var nextState=home?"1":"0";
    if(custom.dataset.spRouteActive!==nextState){
      custom.dataset.spRouteActive=nextState;
      if(home){
        setTimeout(restartAnnouncementMarquee,0);
        setTimeout(restartAnnouncementMarquee,120);
      }
    }
  }
  ...
  function destroy(){
    ...
    document.querySelectorAll(".sp-provider-shortcuts,.sp-banner-dots,.sp-quick-actions,.sp-copy-toast,.sp-keyboard-page-shield,.sp-stable-announcement").forEach(function(item){item.remove();});
    document.documentElement.classList.remove("sp-bank-transfer-selected");
    document.documentElement.classList.remove("sp-independent-announcement-ready");
  }
