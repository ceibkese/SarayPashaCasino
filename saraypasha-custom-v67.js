(function () {
  ...
  function removeHomeBannerExtras(){
    ...
    document.querySelectorAll(".sp-category-strip,.sp-provider-shortcuts,.sp-banner-dots").forEach(function(item){item.remove();});
  }

  function enhanceCategoryStrip(){
    if(!isHomePage()){removeHomeBannerExtras();return;}
    if(document.querySelector(".sp-category-strip")) return;
    var banner=document.querySelector('section[data-mj="widget-banner"]');
    if(!banner) return;
    var language=(window.location.pathname.match(/^\/(tr|en)(?:\/|$)/i)||[])[1]||"tr";
    var items=[
      {label:"SLOT",icon:"▦",href:"/"+language+"/casino"},
      {label:"EGT DIGITAL",icon:"♛",href:"/"+language+"/casino?provider=egt-digital"},
      {label:"CANLI CASINO",icon:"◉",href:"/"+language+"/livecasino"},
      {label:"SPOR",icon:"◆",href:"/"+language+"/sport/demo"},
      {label:"PROMOSYONLAR",icon:"✦",href:"/"+language+"/promotions"}
    ];
    var root=document.createElement("nav");
    root.className="sp-category-strip";
    root.setAttribute("aria-label",language.toLowerCase()==="en"?"Quick categories":"Hızlı kategoriler");
    items.forEach(function(item){
      var link=document.createElement("a");
      link.className="sp-category-strip-card";
      link.href=item.href;
      link.innerHTML='<span class="sp-category-strip-icon" aria-hidden="true">'+item.icon+'</span><span class="sp-category-strip-label">'+item.label+'</span>';
      root.appendChild(link);
    });
    banner.insertAdjacentElement("beforebegin",root);
  }
  ...
  function refresh(){ syncPageMode(); enhanceBankTransfer(); enhance(); enhanceAllTransactionInputs(); enhanceBonusRequestPage(); enhanceQuickActions(); enhanceCategoryStrip(); enhanceProviderShortcuts(); enhanceBannerDots(); enhanceAnnouncementBar(); watchWithdrawalPanel(); watchDepositPanel(); continuePendingAction(); }
  ...
  function destroy(){
    ...
    document.querySelectorAll(".sp-category-strip,.sp-provider-shortcuts,.sp-banner-dots,.sp-quick-actions,.sp-copy-toast,.sp-keyboard-page-shield").forEach(function(item){item.remove();});
    document.documentElement.classList.remove("sp-bank-transfer-selected");
  }
